...
def Main(feature_name, model_name):
    ...
    for count in range(Config.num_k):
        # Data Load
        train_data_dir_list = Load_data(f"{Config.savedir_train_and_test}/Fold_{count}/train_list.txt")
        test_data_dir_list = Load_data(f"{Config.savedir_train_and_test}/Fold_{count}/test_list.txt")
        ...
        num_crackle = train_labels[:, 0].sum().item()  # How many have crackle
        num_wheeze = train_labels[:, 1].sum().item()   # How many have wheeze
        total_samples = train_labels.shape[0]

        print(f"Total samples: {total_samples}")
        print(f"Samples with crackle: {num_crackle}")
        print(f"Samples with wheeze: {num_wheeze}")

        # Calculate pos_weights for BCEWithLogitsLoss
        # pos_weight = (num_negative_samples) / (num_positive_samples)
        num_no_crackle = total_samples - num_crackle
        num_no_wheeze = total_samples - num_wheeze

        pos_weight_crackle = (num_no_crackle / num_crackle) ** 0.5 
        pos_weight_wheeze = (num_no_wheeze / num_wheeze) ** 0.5

        pos_weight_crackle = 1.2
        pos_weight_wheeze = 1.8

        pos_weights = torch.tensor([pos_weight_crackle, pos_weight_wheeze]).to(device)
        ...
        for epoch in range(Config.EPOCH):
            train_info = Train_one_epoch(model, optimizer, train_dataloader, device, epoch,pos_weights)
