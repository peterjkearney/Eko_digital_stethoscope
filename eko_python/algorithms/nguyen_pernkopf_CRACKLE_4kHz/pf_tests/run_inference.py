...
def main() -> None:
        ...
        for tensors, indices in loader:
            ...
            for prob, idx in zip(probs, indices.numpy()):
                ...
                chunk_rows.append({
                    ...
                    'prob_no_crackle': round(float(prob[0]), 4),
                    'prob_crackle':    round(float(prob[1]), 4),
                })

    # ── Aggregate and print ───────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"{'File':<45} {'Pred':<12} {'Conf':>6}  "
          f"{'no_crackle':>10} {'crackle':>7}  N")
    print(f"{'-'*70}")
    ...
    for fname in sorted(file_probs.keys()):
        ...
        print(
            f"{fname:<45} {pred_label:<12} {confidence:>6.3f}  "
            f"{avg_probs[0]:>10.3f} {avg_probs[1]:>7.3f}  {n_chunks}"
        )
        ...
        rows.append({
            ...
            'prob_no_crackle': round(float(avg_probs[0]), 4),
    ...
    fieldnames = ['file', 'patient', 'trial', 'severity', 'prediction',
                  'confidence', 'prob_no_crackle', 'prob_crackle', 'n_chunks']
    ...
    chunk_fieldnames = ['file', 'chunk', 'start_sample', 'end_sample',
                        'prediction', 'prob_no_crackle', 'prob_crackle']
