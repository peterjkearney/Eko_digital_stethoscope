...
# Maps layer name → index in the backbone Sequential up to which layers are frozen.
    ...
    def __init__(
        ...
        if freeze_until is not None:
            if freeze_until not in _FREEZE_UNTIL_INDEX:
                raise ValueError(f"freeze_until must be one of {list(_FREEZE_UNTIL_INDEX)} or None")
...
class CoTuningModel(nn.Module):
    ...
    def trainable_parameters(self) -> list:
        """All parameters with requires_grad=True (excludes source classifier and frozen backbone layers)."""
        return [
            p for p in (
...
def build_model(
    ...
    backbone          = ResNetBackbone(variant=variant, pretrained=pretrained,
                                       freeze_until=FREEZE_BACKBONE_UNTIL)
...
def print_model_summary(model: CoTuningModel) -> None:
    ...
    # Backbone — broken down per sublayer so frozen layers are visible
    bb_total = bb_trainable = 0
    print("  Backbone:")
    for i, layer in enumerate(model.backbone.features):
        name  = _BACKBONE_LAYER_NAMES[i] if i < len(_BACKBONE_LAYER_NAMES) else f'layer_{i}'
        total, trainable = _count(layer)
        if total == 0:
            continue   # relu, maxpool — no parameters
    ...
    for name, total, trainable in other_rows:
        frozen = '  (frozen)' if trainable == 0 else ''
        print(f"  {name:<22}: {trainable:>8,} / {total:>8,} trainable{frozen}")

    total_all     = bb_total     + sum(r[1] for r in other_rows)
    trainable_all = bb_trainable + sum(r[2] for r in other_rows)
    print("-" * 60)
