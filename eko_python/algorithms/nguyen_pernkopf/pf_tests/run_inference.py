...
Runs inference on all preprocessed Eko recordings using the trained model
from checkpoints/final/best.pt.

Each recording is divided into fixed-length chunks. The model produces
softmax probabilities for each chunk; these are averaged across all chunks
of the same recording to give a single per-recording prediction.

Pipeline:
    1. Build model and load checkpoints/final/best.pt
    2. Load EkoInferenceDataset (spectrum correction + VTLP log-mel)
...
CHUNK_RESULTS_CSV = RESULTS_DIR / 'inference_chunk_results.csv'
...
def parse_filename(name: str) -> dict:
    ...
    # severity is everything between trial and date (handles multi-word like 'very_mild')
    # date is the last all-digit token
    date_idx = next((i for i in range(len(parts)-1, -1, -1) if parts[i].isdigit()), None)
...
def main() -> None:
    ...
    parser.add_argument('--no-correction', action='store_true',
                        help='Skip spectrum correction (for diagnostic comparison)')
    args = parser.parse_args()

    suffix = '_nocorr' if args.no_correction else ''
    RESULTS_CSV       = RESULTS_DIR / f'inference_results{suffix}.csv'
    CHUNK_RESULTS_CSV = RESULTS_DIR / f'inference_chunk_results{suffix}.csv'
    ...
    epoch, icbhi_score = load_checkpoint(
        str(CHECKPOINT_PATH), model, optimiser=None, device=device
    )
    model.eval()
    ...
    if len(dataset) == 0:
        print("No chunks found. Check INPUT_DIR and that upsample.py has been run.")
        return
    ...
    # ── Inference ─────────────────────────────────────────────────────────
    # Accumulate softmax probabilities per file and per chunk
    ...
    with torch.no_grad():
        for tensors, bandwidths, indices in loader:
            tensors    = tensors.to(device)
            bandwidths = bandwidths.to(device)
            target_logits, _, _ = model(tensors, bandwidth=bandwidths)
    ...
    # ── Aggregate and print ───────────────────────────────────────────────
    print(f"\n{'='*70}")
    print(f"{'File':<45} {'Pred':<8} {'Conf':>6}  {'normal':>7} {'crackle':>7} {'wheeze':>7} {'both':>7}  {'N'}")
    print(f"{'-'*70}")

    rows = []
    for fname in sorted(file_probs.keys()):
        avg_probs   = np.stack(file_probs[fname]).mean(axis=0)  # (4,)
    ...
    fieldnames = ['file', 'patient', 'trial', 'severity', 'prediction',
                  'confidence', 'prob_normal', 'prob_crackle', 'prob_wheeze',
                  'prob_both', 'n_chunks']
    ...
    with open(CHUNK_RESULTS_CSV, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=chunk_fieldnames)
        writer.writeheader()
        writer.writerows(chunk_rows)
    print(f"Chunk results saved to '{CHUNK_RESULTS_CSV}'.")
