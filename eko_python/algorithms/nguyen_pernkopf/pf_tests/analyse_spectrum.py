...
Method (mirrors spectrum_correction.py):
    ...
    2. Compute mean log power spectrum across all preprocessed Eko recordings.
...
Run upsample.py first — this script reads from preprocessed/.
...
# ---------------------------------------------------------------------------
# Spectrum helpers (mirrors spectrum_correction.py)
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    if not wav_files:
        print(f"No files found in '{INPUT_DIR}'. Run upsample.py first.")
        return

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("EKO DEVICE SPECTRUM ANALYSIS")
    print("=" * 60)
    ...
    freq_bins = librosa.fft_frequencies(sr=SAMPLE_RATE, n_fft=N_FFT)  # Hz
    spectra   = []

    for wav_path in wav_files:
        spectrum = compute_mean_log_power_spectrum(
            wav_path, sr=SAMPLE_RATE, n_fft=N_FFT, hop_length=HOP_LENGTH
        )
    ...
    print(f"\nEko profile estimated from {len(spectra)} recordings.")
    ...
    with open(SPECTRUM_CORRECTION_PROFILES_PATH, 'w') as f:
        json.dump(icbhi_profiles_raw, f, indent=2)
    print(f"\nEko profile saved to '{SPECTRUM_CORRECTION_PROFILES_PATH}'.")

    # ── Plot ─────────────────────────────────────────────────────────────
    fig, axes = plt.subplots(3, 1, figsize=(12, 12))
    fig.suptitle('Eko Device Spectrum Analysis', fontsize=14, fontweight='bold')
    ...
    ax.fill_between(freq_bins, correction_filter, 0,
                    ...
                    label='Boost (Eko weaker than Meditron)')
    ax.fill_between(freq_bins, correction_filter, 0,
                    where=correction_filter < 0, alpha=0.2, color='red',
                    label='Cut (Eko stronger than Meditron)')
