...
Usage:
    ...
    python pf_tests/plot_waveforms.py --no-correction
"""
...
def main() -> None:
    ...
    parser.add_argument('--no-correction', action='store_true',
                        help='Read chunk results from the no-correction inference run')
    args = parser.parse_args()

    suffix    = '_nocorr' if args.no_correction else ''
    chunk_csv = CHUNK_RESULTS_CSV.parent / f'inference_chunk_results{suffix}.csv'
    plots_dir = PLOTS_DIR.parent / f'waveform_plots{suffix}' if args.no_correction else PLOTS_DIR
    ...
    by_file = load_chunk_results(chunk_csv)
    ...
    for fname, chunks in sorted(by_file.items()):
        ...
        if file_sr != SAMPLE_RATE:
            audio = librosa.resample(audio.astype(np.float32),
                                     orig_sr=file_sr, target_sr=SAMPLE_RATE)
            file_sr = SAMPLE_RATE
