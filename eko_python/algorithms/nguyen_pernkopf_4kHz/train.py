...
def train_run(
    ...
    history          = []
    start_epoch      = 1
    overfit_streak   = 0   # consecutive train-eval epochs where gap > OVERFIT_GAP_THRESHOLD
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        if not is_full_train:
            ...
            # Evaluate on train every 5 epochs only — avoids a full forward pass each epoch.
            ...
            if epoch % 5 == 0 or epoch == 1:
                train_icbhi_metrics = evaluate(model, train_loader, device)
                train_score = train_icbhi_metrics['icbhi_score']
            ...
            if overfit_streak >= OVERFIT_GAP_PATIENCE:
                print(
                    f"  Early stopping: train−val ICBHI gap > {OVERFIT_GAP_THRESHOLD:.2f} "
